...
class HListTests {
  ...
  def testSingletonProductArgs: Unit = {
    ...
    typed[Witness.`23`.T :: Witness.`"foo"`.T :: Witness.`Symbol("bar")`.T :: Obj.type :: Witness.`true`.T :: HNil](l)
    ...
    val v3: Witness.`Symbol("bar")`.T = l.tail.tail.head
